...
**Manually-managed dependencies:** `electron`, `electron-builder`, `electron-updater`, and `@sentry/*` are excluded from Dependabot (see `.github/dependabot.yml` ignore rule) and must be updated manually, each for a different reason:
