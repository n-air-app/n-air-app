...
**Manually-managed dependencies:** `electron`, `electron-builder`, `electron-updater`, and `@sentry/*` are excluded from Dependabot (see `.github/dependabot.yml` ignore rule) and must be updated manually. Electron-related packages have wide-reaching impact across native modules and packaging. `@sentry/*` packages require coordinated updates — `@sentry/electron`'s version pairs 1:1 with an internal SDK version shared by the browser-side SDKs (`@sentry/vue`, etc.), but the version numbers themselves don't match (e.g. electron 7.x ↔ vue 10.x), so updating them independently risks a runtime mismatch that Dependabot can't account for.
