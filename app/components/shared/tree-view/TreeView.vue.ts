...
export default defineComponent({
  ...
  methods: {
    ...
    cursorStyle(node: ITreeNode<unknown>, placement: TDropPlacement): CSSProperties {
      const visible = this.cursorPosition?.node.pathStr === node.pathStr && this.cursorPosition?.placement === placement;
      return { visibility: visible ? 'visible' : 'hidden', '--depth': node.level - 1 } as CSSProperties;
    },
    ...
    onNodeDragOver(node: ITreeNode<unknown>, event: DragEvent) {
      ...
      this.cursorPosition = { node, placement: getDropPlacement(!!node.isLeaf, event.clientY - rect.top, rect.height, this.edgeSize) };
    ...
    onRootDragOver(event: DragEvent) {
      ...
      if (node) this.cursorPosition = { node, placement: before ? 'before' : 'after' };
      this.updateAutoScroll(event.clientY);
    },
